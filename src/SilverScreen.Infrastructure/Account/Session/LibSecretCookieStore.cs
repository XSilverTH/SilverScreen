...
///         Interacts with the user's desktop Secret Service provider (e.g., GNOME Keyring, KeePassXC with Secret Service
///         integration)
///         over D-Bus through <c>libsecret-1.so.0</c>. Attributes (<c>application=SilverScreen</c>,
///         <c>credential=youtube-manual-session</c>)
///         identify and isolate the stored session cookies.
///     </para>
///     <para>
///         <b>Lazy Initialization & Startup Non-blocking:</b>
///         Native GLib function exports (<c>g_str_hash</c>, <c>g_str_equal</c>, <c>g_free</c>) and native library handles
///         are lazily resolved
///         on first access via <see cref="NativeLibrary.Load(string)" />. No synchronous connection or keyring unlock
///         prompts occur during static construction,
///         avoiding any synchronous keyring lockups or deadlocks on startup paths.
///     </para>
///     <para>
///         <b>Keyring Thread Expectations:</b>
///         Keyring operations use synchronous native bindings (<c>secret_password_lookupv_binary_sync</c>,
///         <c>secret_password_storev_binary_sync</c>,
///         and <c>secret_password_clearv_sync</c>). When prompts or user unlock interactions occur, the Secret Service
///         daemon blocks the calling thread
///         until the prompt resolves. All native persistence operations are offloaded to background worker tasks (
